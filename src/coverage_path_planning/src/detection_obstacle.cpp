  ...
  if (listenerOK){ // Si le listener est initialisé, on peut analyser le scan de l'Hokuyo
    ...
    for (int i=0; i<scan.ranges.size(); i++){
      if (!isinf(scan.ranges[i])){  // si la valeur du scan est différente de "Inf" alors il y a un obstacle
        ...
        if (info_scan_ranges.empty() && sqrt(pow(obstacle[0],2)+pow(obstacle[1],2))< diametre_tondeuse/2){ //adrien : ajout de la seconde condition
          ...
          bool ajout_info_scan_ranges;
            ...
            if(norme < norme_new_obstacle || norme < diametre_tondeuse/2){ //adrien : ajout de la seconde condition
    ...
    for (int i = obs_repertories.size()-1; i >= 0; i--){
      ...
      if((dist_obs_hokuyo <= range_max_hokuyo) && (abs(angle_obs_hokuyo) <= offset_angle)){
        ...
        // On définit une norme pour être sûr que tous les obstacles détectés sont assez éloignés de l'obstacle répertorié et analysé
        if (norme_obs < norme_obs_dynamique){
          suppr_obstacle = false;        
        }

//adrien : on supprime l'obstacle si celui-ci se trouve à portée du robot
	if (norme_obs < diametre_tondeuse/2){ //si la distance est inférieure au rayon de la tondeuse
          suppr_obstacle = true;        
        }
	if (obstacle[0] < diametre_tondeuse/2 && obstacle[1] < diametre_tondeuse/2){ //points situés dans le robot
          suppr_obstacle = true;
        }
	
	//Filtre pour supprimer les obstacles crés par le bruit ambiant.	
...
// adrien : supprimer les points qui se créent en avançant le robot

	  float obstacle_robot = sqrt(pow(position.x-obstacle[0],2) + pow(position.y-obstacle[1],2));
	  if(obstacle_robot < diametre_tondeuse){
            new_obstacle = false;
          }
...
//adrien : tentative de correction de l'obtacle sur le robot
	//if(sqrt(pow(obs_repertories[i][0]-position.x,2) + pow(obs_repertories[i][1]-position.y,2)) > diametre_tondeuse ){
	if(sqrt(pow(obs_repertories[i][0]-position.x,2)) > diametre_tondeuse || sqrt(pow(obs_repertories[i][1]-position.y,2)) > diametre_tondeuse ){
...
int main(int argc, char *argv[]){
  ...
  ros::Publisher pub1 = detection_obstacle.advertise<sensor_msgs::PointCloud>("/detection_obstacle/coord",1000); // topic qui va permettre de publier la position des obstacles
