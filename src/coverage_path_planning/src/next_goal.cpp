...
/* Ce code permet de planifier la trajectoire du robot.  
*/
/*------------------------------------------------------------------Modifié-par-Luc-ZHENG-le-5-septembre-2018---------------------------------------------------------------------*/
...
void quaternion_ros::toQuaternion(float pitch, float roll, float yaw)
{
  ...
  // Abbréviations pour les différentes fonctions angulaires
  ...
  y = cy * cr * sp + sy * sr * cp;
  z = sy * cr * cp - cy * sr * sp;
  cout << "Valeur du quaternion:" << endl;
...
int main(int argc, char *argv[]){
  srand(time(0));
	...
	ros::Subscriber sub1 = next_goal.subscribe("/odometry/filtered_map",1000,pose_recu);
  ros::Subscriber sub2 = next_goal.subscribe("/path_planned/path",1000,path_recu);

  ros::Publisher pub1 = next_goal.advertise<geometry_msgs::PoseStamped>("path_planned/goal",1000);
